    ...
    for i in range(num_train_iters):
        ...
        if i % eval_step == 0:
            ...
            for crop in enhanced_crops:
                try:
                    # Ensure the data type of test_crops[idx] matches the data type of crop
                    # https://stackoverflow.com/questions/60138697/typeerror-cannot-handle-this-data-type-1-1-3-f4
                    test_crop_reshaped = np.reshape(test_crops[idx], crop.shape) * 255
                    test_crop_reshaped = test_crop_reshaped.astype(np.uint8)
                    crop = crop * 255
                    crop = crop.astype(np.uint8)

                    # Horizontally stack the reshaped test_crops[idx] and crop
                    before_after = np.hstack((test_crop_reshaped, crop))
                    ...
                    print(np.amin(crop))
                    print(np.amax(crop))
                    print("ABOUT test_crop_idx")
                    print(test_crops[idx].shape)
                    print(type(test_crops[idx]))
                    print(np.amin(test_crops[idx]))
                    print(np.amax(test_crops[idx]))
                    print("BEFORE AFTER:", e)
                    print(before_after.shape)
                    print(before_after.shape)
                    print(np.amin(before_after))
                    print(np.amax(before_after))
                    ############################
                    # ERROR
                    # (30000,)
                    # (100, 100, 3)
